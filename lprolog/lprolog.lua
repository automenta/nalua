...
function call_facts(obj,...)
	...
	--print("call:",name,ipairs(arg));
	local query=_G.goal;
	for i,v in ipairs(arg) do
		--print("  ",i,v);
		if (not( v == false)) and (v.type=="value") then
			query=true;
		end
	end
	...
	for i,v in ipairs(_G.facts[name]) do
		if (not( v == false)) and (v.len==len) then
			local flag=true;
			for j,vv in ipairs(v.fact) do
				local aj = arg[j];
				if (not(aj == false)) and aj.vtype~="value" then
					if aj.name~=vv.name then
						flag=false;
					end
				end
			end
...
tprint(father(vader,luke),2)

print("==================");

print(inspect(father(vader, luke)))
...
solution= RULE(function(L1,L2,L3,L4,L5,L6,L7,L8,L9,L10,L11,L12,L13,L14,L15,L16)
	return AND(
			word(L1,L2,L3,L4,L5),
			word(L1,L6,L9,L15),
			word(L3,L7,L11),
